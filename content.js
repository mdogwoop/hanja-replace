...
(function () {
  ...
  // contextText = 周围元素文本(用于 near 规则);after = 紧跟该词其后的若干字符
  // (用于词尾启发式 after 规则,如 고려'하여' → 考慮 而非 高麗)。
  function resolveHanja(hangul, contextText, after) {
    ...
    if (hanja == null) return null;     // 规则判定为不应替换(用言/语法形式等)
  ...
  function appendMarked(wrap, hangul, hanja) {
    if (hangul.length !== hanja.length) {
      // length mismatch (rare) — mark the whole thing
